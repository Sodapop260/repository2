...
private:
    ...
    // ボールの速さ
    static constexpr double speed = 470.0;
    ...
    // パドル
    Rect m_paddle = Rect(Arg::center(Cursor::Pos().x, 450), 60, 10);
...
public:
        ...
        : IScene(init)
    {
        ...
        {
            m_blocks << Rect(p.x * blockSize.x, 100 + p.y * blockSize.y, blockSize);
        }
    ...
    {
        // パドルを操作
        m_paddle = Rect(Arg::center(Cursor::Pos().x, Cursor::Pos().y), 60, 10);
        ...
        //下段にぶつかったら跳ね返る
        if (m_ball.y > Scene::Height()  && m_ballVelocity.y > 0)
        {
            m_ballVelocity.y *= -1;
        }
        ...
        // パドルにあたったらはね返る
        if ((m_ballVelocity.y < 0 || m_ballVelocity.y > 0) && m_paddle.intersects(m_ball))
        {
